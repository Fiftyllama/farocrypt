...
All slicing happens on the last axis, so these functions accept either a single
chunk (1-D array) or a batch of chunks that share the same shuffle parameters
(2-D array, shape (n_chunks, chunk_size)) — a whole round's chunks can be
shuffled in one vectorized call instead of looping chunk by chunk in Python.
"""
...
def shuffle(data: np.ndarray, shuffle_type: str, steps: int, variant: int) -> np.ndarray:
    ...
    # _shuffle_step always allocates a fresh array, so a copy is only needed
    # here if the loop never ran (steps == 0), to preserve the "never
    # returns the input object itself" contract.
    return result.copy() if result is data else result
